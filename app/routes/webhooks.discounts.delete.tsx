...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (topic !== "DISCOUNTS_DELETE" && topic !== "discounts/delete") {
  ...
  try {
    ...
    // ── Strategy 1: Delete by Shopify discount ID ──────────────────────────
    if (shopifyDiscountId) {
      const result = await db.campaign.deleteMany({
        where: { shop, shopifyDiscountId },
      });
      deleteCount = result.count;
      console.log(`[Webhook] ID match "${shopifyDiscountId}": deleted ${deleteCount} campaigns`);
    }

    // ── Strategy 2: Fallback — delete by title ────────────────────────────
    if (deleteCount === 0 && discountTitle) {
      const result = await db.campaign.deleteMany({
        where: { shop, name: discountTitle },
      });
      deleteCount = result.count;
      console.log(`[Webhook] Title match "${discountTitle}": deleted ${deleteCount} campaigns`);
    }

    if (deleteCount === 0) {
      console.warn(`[Webhook] No campaigns matched for deletion. shop=${shop} id=${shopifyDiscountId} title="${discountTitle}"`);
    }
  } catch (error) {
    console.error(`[Webhook] Error processing discount delete:`, error);
  }
