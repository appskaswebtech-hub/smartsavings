  ...
  //   // Popup email signups for this campaign (only relevant when the popup is enabled)
  //   let submissions: {
  //     email: string;
  //     name: string | null;
  //     code: string | null;
  ...
  //     // Best-effort: resolve customer names from Shopify by email (single query).
  //     // The popup only collects an email, so a name only exists for known customers.
  //     const nameByEmail: Record<string, string> = {};
  //     const emails = Array.from(new Set(rows.map((r) => r.email.toLowerCase()))).slice(0, 40);
  //     if (emails.length > 0) {
  //       try {
  //         const q = emails.map((e) => `email:${e}`).join(" OR ");
  //         const res = await admin.graphql(
  //           `#graphql
  //           query resolveNames($q: String!) {
  //             customers(first: 100, query: $q) {
  //               nodes { email firstName lastName }
  //             }
  //           }`,
  //           { variables: { q } }
  //         );
  //         const data = await res.json();
  //         for (const c of data.data?.customers?.nodes || []) {
  //           const nm = [c.firstName, c.lastName].filter(Boolean).join(" ");
  //           if (c.email && nm) nameByEmail[c.email.toLowerCase()] = nm;
  //         }
  //       } catch {}
  //     }
  ...
  //       email: r.email,
  //       name: nameByEmail[r.email.toLowerCase()] || null,
  //       code: r.code,
  ...
  //   const { campaign, tiers, productNames, collectionNames, shopifyDiscounts, submissions } = useLoaderData<typeof loader>();
  ...
  //                 </BlockStack>
  //                 <BlockStack gap="200">
  //                   <Text as="span" variant="bodySm" tone="subdued">Shown on</Text>
  //                   <Text as="span" variant="bodyMd">
  //                     {(() => {
  //                       let pages: string[] = [];
  //                       try { pages = JSON.parse(campaign.popupPages || "[]"); } catch {}
  ...
  //                 </BlockStack>
  //               </InlineGrid>
  //               {campaign.popupFrequencyValue && (
  //                 <BlockStack gap="200">
  //                   <Text as="span" variant="bodySm" tone="subdued">Re-show after closing</Text>
  //                   <Text as="span" variant="bodyMd">
  //                     {campaign.popupFrequencyValue} {campaign.popupFrequencyUnit || "hours"}
  //                   </Text>
  //                 </BlockStack>
  //               )}
  //               {campaign.popupHeading && (
  //                 <BlockStack gap="200">
  //                   <Text as="span" variant="bodySm" tone="subdued">Heading</Text>
  //                   <Text as="span" variant="bodyMd">{campaign.popupHeading}</Text>
  //                 </BlockStack>
  //               )}
  //             </BlockStack>
  //           </Card>
  ...
  //                 <Badge tone="info">{`${submissions.length} ${submissions.length === 1 ? "signup" : "signups"}`}</Badge>
  //               </InlineStack>
  ...
  //                     <span style={{ flex: 1.5 }}>Discount</span>
  //                     <span style={{ flex: 1 }}>Code</span>
  //                     <span style={{ flex: 1.5 }}>Submitted</span>
  //                   </div>
  //                   {submissions.map((s, idx) => (
  //                     <div key={idx} style={{ display: "flex", padding: "10px 16px", fontSize: "13px", borderBottom: idx < submissions.length - 1 ? "1px solid #f0f0f0" : "none", alignItems: "center" }}>
  //                       <span style={{ flex: 2, fontWeight: "600", wordBreak: "break-all" }}>{s.email}</span>
  //                       <span style={{ flex: 1.5, color: s.name ? "inherit" : "#999" }}>{s.name || "—"}</span>
  //                       <span style={{ flex: 1.5, fontSize: "12px", color: "#666" }}>{campaign.name}</span>
  //                       <span style={{ flex: 1 }}>{s.code ? <Badge>{s.code}</Badge> : "—"}</span>
  //                       <span style={{ flex: 1.5, fontSize: "12px", color: "#666" }}>{s.createdAt}</span>
  //                     </div>
  //                   ))}
  //                 </div>
  //               ) : (
...
//       updatedAt: new Date(campaign.updatedAt).toLocaleString(),
//     },
//     tiers, productEntries, shopifyProducts, collectionNames, shopifyDiscounts,
//   });
// };
