...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const config = await db.widgetCustomization.findUnique({ where: { shop_widgetType: { shop: session.shop, widgetType: "countdown_end" } } });
  return json({ config: config ? JSON.parse(config.config) : null });
};
export const action = async ({ request }: ActionFunctionArgs) => {
  const { session } = await authenticate.admin(request);
  const formData = await request.formData();
  await db.widgetCustomization.upsert({ where: { shop_widgetType: { shop: session.shop, widgetType: "countdown_end" } }, update: { config: formData.get("config") as string }, create: { shop: session.shop, widgetType: "countdown_end", config: formData.get("config") as string } });
...
export default function CountdownEnd() {
  ...
  const [removeCtaButton, setRemoveCtaButton] = useState(false);
  const [title, setTitle] = useState(config?.title ?? "Sale ends in");
  ...
  const [closeIconColor, setCloseIconColor] = useState("#D5D5E5");
  const [titleColor, setTitleColor] = useState("#FFFFFF");
  const [countdownBoxText, setCountdownBoxText] = useState("#FFFFFF");
  const [countdownNumber, setCountdownNumber] = useState("#10b981");
  const [ctaBg, setCtaBg] = useState("#F14262");
  ...
  const BannerPreview = () => (
    <div style={{ background: bannerBg, padding: previewDevice === "mobile" ? "6px 8px" : "10px 16px", display: "flex", alignItems: "center", justifyContent: "center", gap: previewDevice === "mobile" ? "6px" : "12px", borderRadius: `${borderRadius}px`, margin: "4px", flexWrap: "wrap" as const }}>
      ...
      <div style={{ display: "flex", gap: "3px" }}>
        {["05", "06", "21", "03"].map((n, i) => (
          <div key={i} style={{ display: timerLabels[["day", "hr", "min", "sec"][i]] ? "flex" : "none", flexDirection: "column" as const, alignItems: "center", gap: "1px" }}>
            ...
            <span style={{ color: countdownBoxText, fontSize: "6px", opacity: 0.7 }}>{["Day", "Hr", "Min", "Sec"][i]}</span>
          </div>
        ))}
      </div>
      ...
      {!removeCloseIcon && <span style={{ color: closeIconColor, fontSize: "12px" }}>✕</span>}
    </div>
  );

  return (
    <Page backAction={{ content: "Customization", url: "/app/customization" }} title="Ends in countdown timer" subtitle="Set new style for ends in countdown timer." primaryAction={{ content: "Save", onAction: handleSave, loading: isSaving, disabled: isSaving }}>
      ...
      <Layout>
        <Layout.Section>
          <BlockStack gap="400">
            ...
            <Card><BlockStack gap="400"><InlineStack align="space-between"><InlineStack gap="200"><span>⚙️</span><Text as="h2" variant="headingSm">General</Text></InlineStack><Button variant="plain" size="slim">Reset to default</Button></InlineStack><Text as="p" variant="bodySm" fontWeight="bold">Countdown position on homepage</Text><InlineStack gap="200"><Button size="slim" variant={position === "top" ? "primary" : undefined} onClick={() => setPosition("top")}>Top</Button><Button size="slim" variant={position === "bottom" ? "primary" : undefined} onClick={() => setPosition("bottom")}>Bottom</Button></InlineStack><Checkbox label="Sticky banner" checked={stickyBanner} onChange={setStickyBanner} /><Checkbox label="Always visible while scrolling" checked={alwaysVisible} onChange={setAlwaysVisible} /><Checkbox label="Remove close icon" checked={removeCloseIcon} onChange={setRemoveCloseIcon} /><Checkbox label="Remove CTA button" checked={removeCtaButton} onChange={setRemoveCtaButton} /></BlockStack></Card>
        ...
        <Layout.Section variant="oneThird">
          <StickyPreview><Card><BlockStack gap="400">
            <InlineStack align="space-between" blockAlign="center" wrap={false}><Text as="h2" variant="headingSm">Preview</Text><InlineStack gap="200"><Button size="slim" variant={previewMode === "homepage" ? "primary" : undefined} onClick={() => setPreviewMode("homepage")}>Homepage</Button><Button size="slim" variant={previewMode === "product" ? "primary" : undefined} onClick={() => setPreviewMode("product")}>Product page</Button></InlineStack></InlineStack>
            ...
            <Box background="bg-surface-secondary" borderRadius="300" padding="200"><DeviceMockup device={previewDevice}>{enabled && <BannerPreview />}<PlaceholderLines count={previewDevice === "mobile" ? 2 : 3} /></DeviceMockup></Box>
