...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  const bxgyDiscountPct = formData.get("bxgyDiscountPct") as string;
  const discountCode = formData.get("discountCode") as string;
  const reDisplayValue = parseInt((formData.get("reDisplayValue") as string) || "24", 10) || 24;
  const reDisplayUnit = (formData.get("reDisplayUnit") as string) || "hours";
...
export default function EditCampaign() {
  ...
  const handleSave = () => {
    ...
    if (type === "advanced_discount_code") {
      ...
      fd.append("reDisplayValue", reDisplayValue);
      fd.append("reDisplayUnit", reDisplayUnit);
      fd.append("popupEnabled", String(popupEnabled));
        ...
        <Layout.Section>
          <BlockStack gap="400">
            ...
            {type === "advanced_discount_code" && (
              <Card>
                <BlockStack gap="400">
                  <Text as="h2" variant="headingMd">Email popup</Text>
                  <Checkbox
                    label="Show a popup to collect shopper email before revealing the discount code"
                  ...
                  {popupEnabled && (
                    <FormLayout>
                      <TextField
                        ...
                        autoComplete="off"
                        helpText="Displayed at the top of the popup."
                      />
                      <TextField
                        ...
                        multiline={3}
                        autoComplete="off"
                        helpText="Explain what the shopper gets by providing their email."
                      />
                      <TextField
                        label="Button label"
                      ...
                      <BlockStack gap="200">
                        ...
                        <Checkbox
                          label="All pages"
                          checked={popupPageScope === "all"}
                          onChange={(checked) => {
                            setPopupPageScope(checked ? "all" : "specific");
                            if (!checked) setPopupPageTypes([]);
                          }}
                        />
                        {[
                          ...
                          <div key={value} style={{ paddingLeft: "24px" }}>
                            <Checkbox
                              label={label}
                              checked={popupPageTypes.includes(value)}
                              disabled={popupPageScope === "all"}
                              onChange={(chk) => {
                                setPopupPageScope("specific");
                                setPopupPageTypes((prev) =>
                                  chk ? [...prev, value] : prev.filter((t) => t !== value)
                                );
                              }}
                            />
                          </div>
                      ...
                      <BlockStack gap="100">
                        ...
                        <InlineStack gap="300" align="start" blockAlign="end">
                          <div style={{ width: "120px" }}>
                            <TextField label="" labelHidden value={reDisplayValue} onChange={setReDisplayValue} type="number" min="1" autoComplete="off" />
                          </div>
                          <div style={{ width: "140px" }}>
                            <Select label="" labelHidden options={[{ label: "Minutes", value: "minutes" }, { label: "Hours", value: "hours" }]} value={reDisplayUnit} onChange={setReDisplayUnit} />
                          </div>
                        </InlineStack>
                        <Text as="p" variant="bodySm" tone="subdued">If a shopper dismisses the popup, it won't show again until this much time has passed. Shoppers who submit their email won't see it again.</Text>
                      </BlockStack>
                    </FormLayout>
                  )}
                </BlockStack>
